<?PHP
	...
	*/
	...
	if(preg_match('/\/(upload)\/(img)\/?/',$_SERVER["REQUEST_URI"])){
		$image->uploadImage();
	}
	else if(preg_match('\/(qrcode)\/[\s\S]+\/?[0-9]*',$_SERVER["REQUEST_URI"])){
		$image->getQRCode();
	}
	else if(preg_match('\/(drop)\/(img)\/[\s\S]+\/?[0-9]*',$_SERVER["REQUEST_URI"])){
		$image->dropImage();
	}
	else{
		$image->setImageRoot("img/img.upload/");
		$image->getImage();
	}
	*/
?>